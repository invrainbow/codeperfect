...
void Debugger::save_single_var(Json_Navigator js, i32 idx, Dlv_Var* dest) {
    ...
    dest->kind = (Go_Reflect_Kind)js.num(js.get(idx, ".kind"));
    dest->len = js.num(js.get(idx, ".len"));
    dest->cap = js.num(js.get(idx, ".cap"));
    dest->address = js.num_u64(js.get(idx, ".addr"));
    dest->kind_name = js.str(js.get(idx, ".realType"));
    dest->flags = js.num(js.get(idx, ".flags"));
...
void Debugger::fetch_variables(int goroutine_id, int frame_idx, Dlv_Frame *frame) {
    ...
    if (vars_idx != -1)
        frame->args = save_list_of_vars(js, vars_idx);
